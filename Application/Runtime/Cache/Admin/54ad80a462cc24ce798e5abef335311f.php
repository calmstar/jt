                        ...
                        <table class="table table-striped table-hover table-responsive">
                            ...
                            <tbody>
                                <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
                                    <td id="box"><input type="checkbox" value="<?php echo ($v["id"]); ?>"> <span><?php echo ($i++); ?></span></td>
                                    <td id="tname"><?php echo ($v["name"]); ?></td>
                                    <td><?php echo (date('Y-m-d H:i:s',$v["rgdate"])); ?></td>
                                    <td id="dis"> <?php if($v["display"] == 0): ?>否<?php else: ?>是<?php endif; ?> </td>
                                    <td>
        ...
        $(function(){
            ...
            $('#add').on('click',function(){
                ...
                layer.prompt(
                    ...
                    function(val, index){  //传入参数（点击确定），才会执行
                        ...
                        //跳转传值，非异步刷新
                        // window.location.href='/manager.php/Course/add/val/'+val;   

                        //底层方法，可选参数多
                        $.ajax({ 
                            url: "/manager.php/Course/add",
                            data:{name:name},
                            type:"POST",
                            datatype:'text',
            ...
            $('body').on('click','#edit',function(){
                ...
                layer.prompt(
                    ...
                    function(val,index){
                        name = val;
                        layer.close(index);
                        $.post("/manager.php/Course/edit",{name:name,id:id},
                            function(data){
                                if(data['status'] == 1){
                                     layer.msg('编辑成功');
                                     nameGet.html(name);
            ...
            $('body').on('click','#deleSingle',function(){
                ...
                layer.confirm('确认删除?',{btn:['确定','取消']}, 
                    function(){
                        $.post("/manager.php/Course/dele",{ids:value},function(data){
                            if(data['status'] == 1){
                                tr.empty();
                                //更改每条记录信息的序号
                ...
                if(id == ''){
                    ...
                    layer.confirm('确认删除？', {btn: ['确定', '取消']}, 
                        function(){   
                            //点击确定后的回调事件          
                            //ajax异步传值
                            $.post("/manager.php/Course/dele",{ids:id},
                                function(data){
                                    if(data['status'] == 1){
